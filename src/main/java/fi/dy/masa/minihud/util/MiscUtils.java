...
public class MiscUtils
{
    ...
    public static void addAxolotlTooltip(ItemStack stack, List<Text> lines)
    {
        ...
        if (tag != null && tag.contains(AxolotlEntity.VARIANT_KEY, Constants.NBT.TAG_INT))
        {
            ...
            if (variantId >= 0 && variantId < AxolotlEntity.Variant.VARIANTS.length)
            {
                ...
                MutableText labelText = Text.literal(I18n.translate("minihud.label.axolotl_tooltip.label"));
                MutableText valueText = Text.literal(I18n.translate("minihud.label.axolotl_tooltip.value", variantName, variantId));
        ...
        if (stackTag != null && stackTag.contains("BlockEntityTag", Constants.NBT.TAG_COMPOUND))
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (entityDataTag.contains("CustomName", Constants.NBT.TAG_STRING))
                {
                    ...
                    lines.add(Math.min(1, lines.size()), Text.literal(I18n.translate("minihud.label.bee_tooltip.name", Text.Serializer.fromJson(beeName).getString())));
                }
            ...
            if (babyCount > 0)
            {
                text = Text.literal(I18n.translate("minihud.label.bee_tooltip.count_babies", String.valueOf(count), String.valueOf(babyCount)));
            }
            else
            {
                text = Text.literal(I18n.translate("minihud.label.bee_tooltip.count", String.valueOf(count)));
            }
    ...
    public static void addHoneyTooltip(ItemStack stack, List<Text> lines)
    {
        ...
        if (tag != null && tag.contains("BlockStateTag", Constants.NBT.TAG_COMPOUND))
        {
            ...
            lines.add(Math.min(1, lines.size()), Text.literal(I18n.translate("minihud.label.honey_info.level", honeyLevel)));
        }
    }
